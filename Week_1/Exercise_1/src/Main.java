...
public class Main {
     ...
     */
    public static void log_student_score() throws IOException{
        int current_student_score;
        String student_name;
        ...
        for(int index = 0; index < queues_size; index++){
            if(student_names[index].equals(student_name)){
                out.println("Ingrese la nota de " + student_names[index]);
                out.print(">>> ");
                current_student_score = Integer.parseInt(in.readLine());
                student_scores[index] = current_student_score;
                break; //Exit to break
            }
        }
    }
